...
abstract class Provider
{
    ...
    {
        $virtual = $this->kirby()->option(
            "bnomei.kart.providers.{$this->name}.virtual",
            true,
        );
    ...
    protected function checkoutValue(
        array $data,
        string $key,
    ...
    protected function checkoutBool(
        array $data,
        string $key,
        bool $default = false,
    ): bool {
    ...
    protected function checkoutAddress(
        string $prefix = '',
        ?array $data = null,
    ): array {
        $data ??= $this->checkoutFormData();
        ...
        $address = array_filter(
            [
                'first_name' => $this->checkoutValue(
                    $data,
                    $prefix.'first_name',
                    $prefix.'first',
                ),
                'last_name' => $this->checkoutValue(
                    $data,
                    $prefix.'last_name',
                    $prefix.'last',
                ),
                'company' => $this->checkoutValue($data, $prefix.'company'),
                'address1' => $this->checkoutValue(
                    $data,
                    $prefix.'address1',
                    $prefix.'address_1',
                ),
                'address2' => $this->checkoutValue(
                    $data,
                    $prefix.'address2',
                    $prefix.'address_2',
                ),
                ...
                'postal_code' => $this->checkoutValue(
                    $data,
                    $prefix.'postal_code',
                    $prefix.'zip',
                ),
                'country' => $this->checkoutValue(
                    $data,
                    $prefix.'country',
                    $prefix.'country_code',
                ),
        ...
        $name = trim(
            strval($address['first_name'] ?? '').
                ' '.
                strval($address['last_name'] ?? ''),
        );
        if ($name !== '') {
            $address['name'] = $name;
        }
    ...
    protected function checkoutBillingAddress(): array
    {
        ...
        $billing = $billingSame
            ? []
            : $this->checkoutAddress('billing_', $data);
    ...
    public function getUserData(?User $user = null): array
    {
        ...
        return $user->$field()->isNotEmpty()
            ? Yaml::decode($user->$field()->value())
            : [];
    }
    ...
    public function findImagesFromUrls(string|array $urls): array
    {
        ...
        $images = $this->kirby()
            ->site()
            ->kart()
            ->page(ContentPageEnum::PRODUCTS)
            ->images();

        return array_filter(
            array_map(
                // fn ($url) => $this->kart()->option('products.page').'/'.F::filename($url), // simple but does not resolve change in extension
                fn ($url) => Str::startsWith($url, 'file://')
                    ? $url
                    : $images
                        ->filter('name', F::name($url))
                        ->first()
                        ?->uuid()
                        ->toString(), // slower but better results
                $urls,
            ),
        );
    }
    ...
    public function handleWebhook(
        ...
        return WebhookResult::ignored(
            'Webhooks not implemented for '.$this->name,
        );
    }
