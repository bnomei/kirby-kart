...
class Kart implements Kerbs
{
    ...
    public static function checkSignature(
        ...
        return hash_equals($signature, self::signature($url, $without));
    }

    public function validateSignatureOrGo(
        string $redirect = '/',
        ?string $url = null,
    ): void {
        if (! kart()->option('crypto.signature')) {
        ...
        if (
            ! is_string($signature) ||
            self::checkSignature($signature, $url) === false
        ) {
            go($redirect);
        }
    }
    ...
    public static function formatNumber(
        ...
        return $prefix.
            self::formatter(NumberFormatter::DECIMAL)->format($number);
    }
    ...
    public static function formatter(?int $style = null): NumberFormatter
    {
        ...
        if (
            ...
            $kirby->plugin('bnomei/kart')->license()->status()->value() !==
                'active'
        ) {
            $locale = 'ja_JP';
        }
    ...
    public static function formatCurrency(float $number): string
    {
        ...
        return self::formatter(NumberFormatter::CURRENCY)->formatCurrency(
            $number,
            $currency,
        ) ?:
            '';
    ...
    public function message(
        ...
        if ($message === null) {
            return $this->kirby()
                ->session()
                ->pull('bnomei.kart.message-'.$channel);
        }

        $this->kirby()
            ->session()
            ->set('bnomei.kart.message-'.$channel, $message);
    ...
    public function createOrUpdateCustomer(
        ...
        if ($id) {
            $customer = $this->kirby()
                ...
                ->filterBy(
                    fn (
                        $user, // customerId to align with KLUB
                    ) => $user->isCustomer() &&
                        $user->userData('customerId') === $id,
                )
                ->first();
        }
        ...
        if (
            ! $customer &&
            V::email($email) &&
            $this->option('customers.enabled')
        ) {
            $customer = $this->kirby()->impersonate(
                'kirby',
                fn () => $this->kirby()
                    ->users()
                    ->create([
            ...
            if ($events) {
                $this->kirby()->trigger('kart.user.created', [
                    'user' => $customer,
                ]);
            }
        }
    ...
    public function provider(): Provider
    {
        if (! $this->provider) {
            ...
            if ($class === 'kirby') {
                // should have been kirby_cms
                $class = Kirby::class;
            }
            if (
                in_array(
                    strtolower($class),
                    array_map(fn ($i) => $i->value, ProviderEnum::cases()),
                )
            ) {
    ...
    public function ordersWithProduct(ProductPage|string|null $product): Pages
    {
        return $this->orders()->filterBy(
            fn (OrderPage $orderPage) => $product
                ? $orderPage->hasProduct($product)
                : false,
        );
    }
    ...
    public function ordersWithCustomer(User|string|null $user = null): Pages
    {
        ...
        return $this->orders()->filterBy(
            fn (OrderPage $orderPage) => $user &&
                $orderPage->customer()->toUser()?->is($user),
        );
    }
     ...
     * @kql-allowed
     */
    public function ordersWithInvoiceNumber(
        int|string $invoiceNumber,
    ): OrderPage|Page|null {
        ...
        return $this->orders()
            ->filterBy(
                fn (OrderPage $orderPage) => $orderPage->invnumber()->toInt() ===
                    $invoiceNumber,
            )
            ->first();
    }
    ...
    public function productsByParams(array $params = []): Pages
    {
        ...
        if (is_int($expire)) {
            ...
            return new Pages(
                array_filter(
                    kirby()
                        ->cache('bnomei.kart.products')
                        ->getOrSet(
                            'products-'.$key,
                            fn () => array_values(
                                $this->getProductsByParam($params)->toArray(
                                    fn (ProductPage $product) => $product
                                        ->uuid()
                                        ->toString(),
                                ),
                            ),
    ...
    {
        return $this->products()->filterBy(
            fn (ProductPage $page) => ! is_numeric($page->stock()),
        );
    }
    ...
    public function productsWithCategory(
        ...
        $categories = array_unique(
            array_map(fn ($cat) => urldecode($cat), $categories),
        );
            ...
            return new Pages(
                array_filter(
                    kirby()
                        ...
                        ->getOrSet(
                            $key,
                            function () use ($categories, $any) {
                                $products = $any
                                    ? $this->products()->filterBy(
                                        'categories',
                                        'in',
                                        $categories,
                                        ',',
        ...
        return $any
            ? $this->products()->filterBy('categories', 'in', $categories, ',')
            : $this->products()->filterBy(
                fn ($product) => count(
                    array_diff($categories, $product->categories()->split()),
                ) === 0,
            );
    }
    ...
    public function tags(?string $path = null): Collection
    {
        ...
        if (is_int($expire)) {
            $key = Kart::hash(
                implode(
                    ',',
                    array_filter([$path, $this->category(), $this->tag()]),
                ),
            );
            $tags = kirby()
                ->cache('bnomei.kart.tags')
                ->getOrSet(
                    'tags-'.$key,
                    fn () => $this->getTags($path),
                    $expire,
                );
        } else {
    ...
    public function tag(bool $multiple = false): ?string
    {
        ...
        $t = array_map(
            fn ($tag) => trim(strip_tags(urldecode(strval($tag)))),
            $t,
        );
        $t = array_filter(
            $t,
            fn ($tag) => ! empty($tag) && in_array($tag, $tags),
        );
        if (empty($t)) {
            return null;
        }
            ...
            fn ($t) => [
                ...
                'count' => $category
                    ? $products
                        ->children()
                        ->filterBy('tags', $t, ',')
                        ->filterBy('categories', 'in', [$category], ',')
                        ->count()
                    : $products->children()->filterBy('tags', $t, ',')->count(),
                'value' => $t,
                'isActive' => $t === $tag,
                'url' => ($path ? url($path) : $products->url()).
                    '?tag='.
                    urlencode($t),
                'urlWithParams' => url($path ?? $products->id(), [
                    'params' => [
                        'category' => $category,
                        'tag' => $t === $tag ? null : $t,
                    ],
    ...
    public function categories(?string $path = null): Collection
    {
        ...
        if (is_int($expire)) {
            ...
            $categories = kirby()
                ->cache('bnomei.kart.categories')
                ->getOrSet(
                    'categories-'.$key,
                    fn () => $this->getCategories($path),
                    $expire,
                );
        } else {
        ...
        return new Collection(
            array_map(fn ($c) => new Category($c), $categories),
        );
    }
    ...
    public function category(bool $multiple = false): ?string
    {
        ...
        $c = array_map(
            fn ($cat) => trim(strip_tags(urldecode(strval($cat)))),
            $c,
        );
        $c = array_filter(
            $c,
            fn ($cat) => ! empty($cat) && in_array($cat, $categories),
        );
        if (empty($c)) {
            return null;
        }
        ...
        return array_map(
            fn ($c) => [
                ...
                'count' => $tag
                    ? $products
                        ->children()
                        ->filterBy('categories', $c, ',')
                        ->filterBy('tags', 'in', [$tag], ',')
                        ->count()
                    : $products
                        ->children()
                        ->filterBy('categories', $c, ',')
    ...
    public function productsWithTag(string|array $tags, bool $any = true): Pages
    {
        ...
        return $any
            ? $this->products()->filterBy('tags', 'in', $tags, ',')
            : $this->products()->filterBy(
                fn ($product) => count(
                    array_diff($tags, $product->tags()->split()),
                ) === 0,
            );
    }
    ...
    {
        return kirby()->session()->get('bnomei.kart.checkout_form_data', []);
    }
